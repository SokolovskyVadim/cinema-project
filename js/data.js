...
function createMockData (numArrLength) {
    let result = [...Array(numArrLength).keys()].map((item, index) => (item = {
            titles:titles[index],
            releseDate: randomDate(new Date(2018, 0, 1), new Date()) ,
...
function createFilmsCard(obj) {
    ...
    let { titles, posters, rating, releseDate, directors, plot, boxOffice } = obj;
    ...
    cloneDiv.querySelector('.film-info__release-date .film-info__text').textContent = releseDate;
